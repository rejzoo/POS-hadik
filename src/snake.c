...
void snakeGrow(Snake *snake) {
  if (snake->n_body < MAX_LENGTH_SNAKE) {
    snake->body[snake->n_body++] =
        (Position){snake->x - snake->dx, snake->y - snake->dy};
  }
}
...
void snakeSetAlive(Snake *snake, int status) { snake->alive = status; }

void snakeSetPause(Snake *snake, int status) { snake->pause = status; }

int snakeAlive(const Snake *snake) { return snake->alive; }

Position snakeGetHead(const Snake *snake) {
  return (Position){snake->x, snake->y};
}

int snakeGetX(Snake *snake) { return snake->x; }

int snakeGetY(Snake *snake) { return snake->y; }

int snakeGetBodyLength(Snake *snake) { return snake->n_body; }

Position snakeGetBodyPart(Snake *snake, int index) {
  return snake->body[index];
}

int snakeGetSocket(Snake *snake) { return snake->socket; }

int snakeGetPause(Snake *snake) { return snake->pause; }
