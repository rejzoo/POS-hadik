...
void Client_startServer(char *map_size, char *max_clients, char *game_mode,
  ...
  printf("PID: %d\n", pid);
  if (pid == 0) {
    execl("./SnakeGameServer", "./SnakeGameServer", map_size, max_clients,
          game_mode, game_time, NULL);
...
void Client_parseData(const char *data1, const char *data2,
  ...
  for (int i = 0; i < serverData->n_snakes; i++) {
    sscanf(ptr, "%d %d", &serverData->snakes_heads[i].x,
           &serverData->snakes_heads[i].y);
    ptr = strchr(ptr, ' ') + 1;
    ptr = strchr(ptr, ' ') + 1;
  }

  for (int i = 0; i < serverData->n_bodies; i++) {
    sscanf(ptr, "%d %d", &serverData->snakes_bodies[i].x,
           &serverData->snakes_bodies[i].y);
    ptr = strchr(ptr, ' ') + 1;
    ptr = strchr(ptr, ' ') + 1;
  }
...
void *Client_receiveUpdates(void *arg) {
  ...
  while (client->client_alive) {
    ssize_t bytes_received1 =
        recv(client->client_fd, buffer, sizeof(buffer) - 1, 0);
    ...
    // ssize_t bytes_received2 = recv(client->client_fd, clientBuffer,
    // sizeof(clientBuffer) - 1, 0); if (bytes_received2 > 0) {
...
void Client_joinGame(Client *client) {
  ...
  if (connect(client->client_fd, (struct sockaddr *)&serv_addr,
              sizeof(serv_addr)) < 0) {
...
int main() {
  ...
  Client_handleChoice(&client, choice, map_size, max_clients, game_mode,
                      game_time);
